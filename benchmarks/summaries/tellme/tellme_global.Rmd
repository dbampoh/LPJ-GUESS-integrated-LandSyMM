...
#author: "Matthew Forrest (script developer and maintainer)"
...
params:
  ...
  old_name: NULL
  data_directory: NULL
  land_cover_file: NULL
  config_file: NULL
  author: NULL
  # "/home/b/b380710/LPJ-GUESS/source/benchmark_tellme_with_dfluxes/benchmarks/exoprocesses/tellme/tellme_global_config.yml"
  #config_file: "/dmz/home/mforrest/Tools/DGVMBenchmarks/inst/tellme_global_config.yml"
---
...
# read and parse the YAML file, then set the objects for later
if(is.null(params$config_file)){
  config_file <- file.path(system.file("tellme_global_config.yml", package = "DGVMBenchmarks")) 
} else { 
  # TODO warn if files missing
  config_file <- params$config_file 
}



all_settings <- parseConfigFile(config_file, cli_args = params)
settings <- all_settings$settings
simulation_sources <- all_settings$simulations
benchmarks <- all_settings$benchmarks

# this override can eb done after
if(!is.null(params$land_cover_file)) settings$land_cover_file <- params$land_cover_file
...
do_global_LAI <- TRUE
do_global_BA <- TRUE
do_Global_Biomass <- FALSE # requires land use
do_Pan_Biomass <- FALSE # requires land use
do_Regrowth <- FALSE # requires special "regrowth" simulation
...
## Hide Settings
Click the tabs above to see the settings of this Tellme script run.

## Models runs
```{r print sources, echo = FALSE}
print(simulation_sources)
```
...
summary_col_names <- c("Quantity", "Period", "Unit")
...
This benchmark compares the simulated net biome productivity (NBP) to estimates from the Global Carbon project (GCP, currently the 2024 budget). The data used for comparison is sum of columns `land sink + budget imbalance - land-use change`.


```{r gcp, fig.height=5}

if("GCP_NBP" %in% names(benchmarks)){
  if(benchmarks[["GCP_NBP"]]@run_benchmark){
    
    overall_tables <- benchmark_GCP(simulation_sources = simulation_sources,
                                    this_benchmark = benchmarks[["GCP_NBP"]],
                                    settings = settings,
                                    tables_list = overall_tables,
                                    summary_col_names = summary_col_names)
  }
}
...
if("PNV_Biomes" %in% names(benchmarks)){
  if(benchmarks[["PNV_Biomes"]]@run_benchmark){
    
    this_benchmark <- benchmarks[["PNV_Biomes"]]
    ...
    #### READ DATASET ####
    full_file_path <- file.path(this_benchmark@datasets[["HandP_biomes"]]@dir, paste0(this_benchmark@guess_layers,".nc"))
    if(file.exists(full_file_path)) {
      
      suppressWarnings(
        Biome_Map_Fields_list[[this_benchmark@datasets[["HandP_biomes"]]@name]] <- getField(source = this_benchmark@datasets[["HandP_biomes"]],
                                                                                            quant = this_benchmark@guess_layers,
      ...
      if(got_simulation){
        ...
        suppressWarnings(
          Biome_Map_Fields_list[[this_sim_Source@id]] <- getScheme(source = temp_simulation_Source,
                                                                   ...
                                                                   first.year = this_benchmark@first.year,
                                                                   last.year = this_benchmark@last.year,
                                                                   year.aggregate.method = this_benchmark@year.aggregate.method,
        ...
        message("Modelled biomes file not found, benchmark will be skipped.")
        do_Biomes <- FALSE
      }
...
if(do_global_GPP){
  ...
  GOSIF_GPP_monthly_file.name <- "GOSIF_GPP_monthly_0.5deg_v4.0.nc"
  GOSIF_GPP_Source <- defineSource(id = "GOSIF_GPP",
                                   name = "GOSIF GPP",
                                   dir = file.path(settings$data_directory, GOSIF_GPP_monthly_dir),
                                   format = NetCDF)
  ...
  if(length(GPP_data_Fields) == 0) {
    ...
    # remove data Field now that they are stored in this_benchmark
    rm(GPP_data_Fields);
    ...
    all_GPP_comparisons <- fullSpatialComparison(this_benchmark, all_GPP_Maps, all_GPP_Trends, all_GPP_Seasonal, settings$new_name, settings$old_name)
...
```{r echo=FALSE}
if(do_global_GPP){
rm( all_GPP_Maps, all_GPP_Trends, all_GPP_Seasonal, all_GPP_comparisons); gc()
}
```
...
if(do_global_LAI){
  ...
  MODIS_LAI_Source <- defineSource(id = "MODIS_LAI",
                                   name = "MODIS LAI",
                                   dir = file.path(settings$data_directory, MODIS_LAI_monthly_dir),
                                   format = GUESS)
  ...
  if(length(LAI_data_Fields) == 0) {
    ...
    # do all spatial comparisons and store them in a big list'o'lists to be plotted in the next code chunks
    all_LAI_comparisons <- fullSpatialComparison(this_benchmark, all_LAI_Maps, all_LAI_Trends, all_LAI_Seasonal, settings$new_name, settings$old_name)
...
```{r echo=FALSE}
if(do_global_LAI){
rm( all_LAI_Maps, all_LAI_Trends, all_LAI_Seasonal, all_LAI_comparisons); gc()
}
```
...
if(do_global_BA){
  ...
  GFED4_BA_Source <- defineSource(id = "GFED4_BA",
                                  name = "GFED4 BA",
                                  dir = file.path(settings$data_directory, GFED4_BA_monthly_dir),
                                  format = NetCDF)
  ...
  if(length(BA_data_Fields) == 0) {
    ...
    # do all spatial comparisons and store them in a big list'o'lists to be plotted in the next code chunks
    all_BA_comparisons <- fullSpatialComparison(this_benchmark, all_BA_Maps, all_BA_Trends, all_BA_Seasonal, settings$new_name, settings$old_name)
...
```{r fluxnet, echo=FALSE, out.width = '100%', results = 'asis', tidy = FALSE}

if("FLUXNET" %in% names(benchmarks)){
  if(benchmarks[["FLUXNET"]]@run_benchmark){
    
    this_benchmark <- benchmarks[["FLUXNET"]]
    
    # read the custom values 
    fluxes_to_benchmark <- this_benchmark@custom$do_fluxes
    measurement_days_reqrd <- this_benchmark@custom$measurement_days_reqrd
    
    for(this_flux_name in fluxes_to_benchmark) {
      
      all_FLUXNET_Fields <- list()
      
      FLUXNET_site_flux_Fields <- getField(this_benchmark@datasets[["FLUXNET"]], quant = this_flux_name)
      
      # # read the model output
      
      all_FLUXNET_Fields[["FLUXNET"]] <- FLUXNET_site_flux_Fields
      
      
      # loop through model runs to be processed
      for(this_sim_Source in simulation_sources) {
        
        if(this_flux_name == "GPP") {
          this_guess_var <- "dgpp"
          this_guess_layer <- "Total"
        }
        else if(this_flux_name == "NEE") {
          this_guess_var <- "dnee"
          this_guess_layer <- "NEE"
        }
        else if(this_flux_name == "Reco") {
        ...
        if(file.exists(file.path(this_benchmark_run_dir, paste0(this_guess_var, ".out"))) || file.exists(file.path(this_benchmark_run_dir,  paste0(this_guess_var, ".out.gz")))) {
          ...
          # read the full data and
          suppressWarnings(
            this_sim_field <- getField(source = this_FLUXNET_Source,
                                       this_guess_var,
          ...
          renameLayers(this_sim_field, this_guess_layer, this_flux_name)
          # convention - positive NEE means absorbed C, but this requires reversing LPJ-GUESS output
          if(this_flux_name == "NEE") this_sim_field <- layerOp(x = this_sim_field,  operator = "mulc", layers = this_flux_name, new.layer = this_flux_name, constant = -1)
          this_sim_field@quant <- FLUXNET_site_flux_Fields@quant
      ...
      decimal_places <- 3
      for(this_field_name in names(all_FLUXNET_Fields)) {
        
        # print(all_FLUXNET_Fields[["FLUXNET"]])
        temp_data_Field <- all_FLUXNET_Fields[[this_field_name]]
        ...
        if(this_field_name == "FLUXNET") temp_data_Field@spatial.extent <- temp_data_Field@spatial.extent[ , Lon := round(Lon, decimal_places)]
        if(this_field_name == "FLUXNET") temp_data_Field@spatial.extent <- temp_data_Field@spatial.extent[ , Lat := round(Lat, decimal_places)]
        all_FLUXNET_Fields[[this_field_name]] <- temp_data_Field
      }
      ...
      # Loop across all sites and make the plots
      
      # list of plot for plotting later
      all_ts_val_combi_plots <- list()
      all_ts_diff_combi_plots <- list()
      all_xy_combi_plots <- list()
      site_summary_rows <- list()
      ...
      for(site_number in 1:nrow(all_FLUXNET_Fields[["FLUXNET"]]@spatial.extent)){
        ...
        this_site_code <- this_site[1, Code]
        this_site_name <- this_site[1, Name]
        # for saving summary numbers for this site
        this_site_summary_row <- copy(this_site)
        
        this_data_site <- selectGridcells(all_FLUXNET_Fields[["FLUXNET"]], c(this_site$Lon, this_site$Lat), spatial.extent.id = this_site$Name)
        this_data_site <- selectLayers(this_data_site, this_flux_name)
        
        # aggregate to 5-day periods
        daily_to_5_days <- c(((0:364) %/% 5) +1)
        this_data_site_dt <- copy(this_data_site@data)[, Day5 := daily_to_5_days[Day]]
        this_data_site_dt[ , Ndays := ifelse(is.na(get(this_flux_name)), 0, 1)]
        this_data_site_5d_dt <- this_data_site_dt[,lapply(.SD, mean, na.rm  = TRUE), by=c("Day5", "Year")]
        # clean a little by requiring  that at least 3 of the 5 days have measurements
        this_data_site_5d_dt[ , (this_flux_name) := ifelse(Ndays >= measurement_days_reqrd/5, get(this_flux_name), NA)]
        this_data_site_5d_dt[ , Day5 := NULL]
        #this_data_site@data <- setKeyDGVM(this_data_site_dt)
        this_data_site_5d <- this_data_site
        this_data_site_5d@data <- setKeyDGVM(this_data_site_5d_dt)
        
        all_comps <- list()
        for(this_name in names(all_FLUXNET_Fields)){
          if(this_name != "FLUXNET") {
            this_model_site <- selectGridcells(all_FLUXNET_Fields[[this_name]], c(this_site$Lon, this_site$Lat), spatial.extent.id = this_site$Name)
            if(nrow(this_model_site@data) > 0 ) {
              this_model_site_dt <- copy(this_model_site@data)[, Day5 := daily_to_5_days[Day]]
              this_model_site_5d_dt <- this_model_site_dt[,lapply(.SD, mean, na.rm  = TRUE), by=c("Day5", "Year")]
              this_model_site_5d_dt[ , Day5 := NULL]
              #print(this_model_site_dt[["GPP"]])
              this_model_site_5d <- this_model_site
              this_model_site_5d@data <- setKeyDGVM(this_model_site_5d_dt)
              
              
              this_comparison <- compareLayers(this_model_site_5d, this_data_site_5d, this_flux_name, area = FALSE, tolerance = 3, show.stats = FALSE)
              all_comps[[this_name]] <- this_comparison
              
              # remove NA 5-day periods   
              this_comparison_no_na <- na.omit(this_comparison@data) 
              this_site_summary_row[ , (this_name) := mean(this_comparison_no_na[[5]])/5]
              this_site_summary_row[ , FLUXNET := mean(this_comparison_no_na[[6]])/5]
              # this_site_summary_row[ ,(paste(this_name, "R2")) := this_comparison@stats$r2]
              
              
              
            } # if model has output
          } # if not the data Field
        } # for each simulation
        ...
        if(length(all_comps) > 0 ){
          ...
          suppressWarnings(all_ts_val_combi_plots[[this_site_code]] <- plotTemporalComparison(all_comps, 
                                                                                              type = "val", 
                                                                                              col.by = "Source",
                                                                                              title = this_site_name,
                                                                                              subtitle = paste("5-day", this_flux_name, "values"),
                                                                                              text.multiplier = settings$map_text_multiplier))
          # Time series of differences
          suppressWarnings(all_ts_diff_combi_plots[[this_site_code]] <- plotTemporalComparison(all_comps, 
                                                                                               type = "diff", 
                                                                                               col.by = "Source",
                                                                                               title = this_site_name,
                                                                                               subtitle = paste("5-day", this_flux_name, "difference from FLUXNET"),
                                                                                               text.multiplier = settings$map_text_multiplier))
          
          # X-Y scatter of 5 day period
          suppressWarnings(xy_plot <- plotXYComparison(all_comps,
                                                       text.multiplier = settings$map_text_multiplier, 
                                                       fit_line_col = "red", 
                                                       perfect_line_col = "cyan", 
                                                       matchLimits = TRUE,
                                                       col.by = "Day"))
          xy_plot <- xy_plot + labs(title = paste(this_flux_name, this_site_name), 
                                    x = bquote(FLUXNET~.(this_flux_name)~"("*kgC~m^{"-2"}~day^{-1}*")"),
                                    y = bquote(Simulated~.(this_flux_name)~"("*kgC~m^{"-2"}~day^{-1}*")"),
                                    subtitle = element_blank())
          xy_plot <- xy_plot + scale_color_gradientn(colors = pals::ocean.phase(12),
                                                     limits = c(1,365),
                                                     breaks = c(1, 91, 182, 273, 365),
                                                     labels = c(1, 91, 182, 273, 365))
          all_xy_combi_plots[[this_site_code]] <- xy_plot
          
          
          # save the summary row
          site_summary_rows[[this_site_code]] <- this_site_summary_row
      ...
      # combine to make big summary table
      site_summary_dt <- rbindlist(site_summary_rows)
      cat("\n")
      # Make big scatter plot
      cat("##", this_flux_name, "Summary \n")
      cat("\n")
      simulation_names <- names(all_FLUXNET_Fields)[-which(names(all_FLUXNET_Fields) == "FLUXNET")]
      site_summary_melted_dt <- melt(site_summary_dt, measure.vars = simulation_names, value.name = "Simulated", variable.name = "Simulation")
      site_summary_melted_dt[ , abs_Lat := abs(Lat)] 
      
      site_summary_melted_dt[ , FLUXNET := FLUXNET * KG_TO_G]
      site_summary_melted_dt[ , Simulated := Simulated * KG_TO_G]
      axis_lims <- range(with(site_summary_melted_dt, c(FLUXNET, Simulated)))
      
      
      all_sites_scatter_plot <- ggplot(site_summary_melted_dt, aes(x = FLUXNET, y = `Simulated`)) +  geom_abline(slope = 1) + geom_point(aes(col = abs_Lat)) + facet_wrap(~Simulation) 
      all_sites_scatter_plot <- all_sites_scatter_plot +  geom_text_repel(aes(label=Code,
                                                                              col = abs_Lat), 
                                                                          size = 4,
                                                                          max.overlaps = 50)
      all_sites_scatter_plot <- all_sites_scatter_plot + coord_fixed(xlim = axis_lims, ylim = axis_lims) + scale_color_viridis(name = "|Lat|")
      all_sites_scatter_plot <- all_sites_scatter_plot + theme(text = element_text(size = theme_get()$text$size * settings$map_text_multiplier * 0.7))
      all_sites_scatter_plot <-  all_sites_scatter_plot + labs(x = bquote(FLUXNET~.(this_flux_name)~"("*gC~m^{"-2"}~day^{-1}*")"),
                                                               y = bquote(Simulated~.(this_flux_name)~"("*gC~m^{"-2"}~day^{-1}*")"),
                                                               title = paste("Mean daily", this_flux_name, "per site (only measurement days)"),
                                                               subtitle = paste("Includes only 5-day periods with sufficient measurements ( >=", measurement_days_reqrd, "/5)"),
                                                               caption = "Data do not include complete years, so numbers are not representative of annual values.")
      
      
      print(all_sites_scatter_plot)
      
      
      
      cat("\n")
      cat("\n")
      cat("##", this_flux_name, "by site {.tabset} \n")
      cat("\n")
      for(this_site_code in names(all_ts_val_combi_plots)){
        cat("\n")
        cat("###", this_site_code , "\n")
        suppressWarnings(print(all_xy_combi_plots[[this_site_code]]))
        cat("\n")
        suppressWarnings(print(all_ts_val_combi_plots[[this_site_code]]))
        cat("\n")
        suppressWarnings(print(all_ts_diff_combi_plots[[this_site_code]]))
        cat("\n")
      }   
      cat("\n")
...
if(do_Global_Biomass){
  ...
  full_file_path <- file.path(settings$data_directory, liu_biomass$this_data_dir, liu_biomass$this_data_file_name)
  if(file.exists(full_file_path)) {
    
    # Read th data via a GUESS formatted Source
    data_Source <- defineSource(id = liu_biomass$dataset_id,
                                name = liu_biomass$dataset_name,
                                dir = file.path(settings$data_directory, liu_biomass$this_data_dir),
                                format = GUESS)
    suppressWarnings(
  ...
  all_lc_dt <- fread(settings$land_cover_file)
  ...
  all_Biomass_comparisons <- fullSpatialComparison(this_benchmark, all_maps = all_Biomass_Maps, new_model = settings$new_name, old_model = settings$old_name)
...
### C Pools per PFT (Pg C)

``` {r C pools per PFT, echo=FALSE, out.width = '100%', out.height= '150%'}
if(do_Global_Summary_Tables) {
  this_complete_df <- modelSummaryTable(simulations = simulation_sources, 
                                        settings = settings,
                                        var = "cmass", 
...
``` {r GPP per PFT, echo=FALSE, out.width = '100%', out.height= '150%'}
if(do_Global_Summary_Tables) {
  this_complete_df <- modelSummaryTable(simulations = simulation_sources, 
                                        settings = settings,
                                        var = "agpp", 
...
``` {r Density, echo=FALSE, out.width = '100%', out.height= '150%'}
if(do_Global_Summary_Tables) {
  this_complete_df <- modelSummaryTable(simulations = simulation_sources, 
                                        settings = settings,
                                        var = "dens", 
...
``` {r AET, echo=FALSE, out.width = '100%', out.height= '150%'}
if(do_Global_Summary_Tables) {
  this_complete_df <- modelSummaryTable(simulations = simulation_sources, 
                                        settings = settings,
                                        var = "aaet", 
...
```   {r Soil N2O, echo=FALSE, out.width = '100%', out.height= '150%'}

### No output here, just calculating the line for the table

### Stefan suggests that the values should be put in a table here, not hardcoded.
### Consider this when restructuring

comparison_period <- c(2000,2010)
...
this_soil_N2O_df <- modelSummaryTable(simulations = simulation_sources, 
                                      settings = settings,
                                      var = "soil_nflux", 
                                      periods = list(comparison_period), 
                                      spatial_extent = settings$spatial_extent,
                                      spatial_extent_id = settings$spatial_extent_id,
                                      area_unit = "ha",
                                      unit_multiplier = KG_TO_TG)
...
summary_N2O_line$Period <- paste(comparison_period[1],  comparison_period[2], sep = "-")
for(this_sim in simulation_sources) {
  summary_N2O_line[[this_sim@name]] <- this_soil_N2O_df[which(this_soil_N2O_df$Source == this_sim@name), "N2O"]
}

names(summary_N2O_line) <- names(overall_tables[["totals"]])
overall_tables[["totals"]] <- rbind(overall_tables[["totals"]], summary_N2O_line)
```
...
# this is used for automatic parameterisation procedures (for example) 
# loop through model runs to be processed
for(this_sim_Source in simulation_sources) {
