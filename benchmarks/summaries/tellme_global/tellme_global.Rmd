...
author: 
  - "Benchmarks run by: `r params$author`"
  - "Tellme global script development: Matthew Forrest"
...
params:
  sim_directory: NULL
  sim_name: NULL
  reference_sim_directory: NULL
  reference_sim_name: NULL
  data_directory: NULL
...
do_Pan_Biomass <- TRUE 
do_Regrowth <- FALSE # requires special "regrowth" simulation which is not currently implemented
      ...
      suppressWarnings(
        Biome_Map_Fields_list[[this_benchmark@datasets[["HandP_biomes"]]@name]] <- getField(source = this_benchmark@datasets[["HandP_biomes"]],
                                                                                            ...
                                                                                            quick.read.file = switch(settings$quick_read+1,
                                                                                                                     NULL,
                                                                                                                     paste(this_benchmark@guess_var, version_label, sep = "_")))
      )
...
if(do_global_GPP){
  ...
  if(length(GPP_data_Fields) == 0) {
    ...
    all_GPP_comparisons <- fullSpatialComparison(this_benchmark, all_GPP_Maps, all_GPP_Trends, all_GPP_Seasonal, settings$reference_sim_name)
...
if(do_global_LAI){
  ...
  if(length(LAI_data_Fields) == 0) {
    ...
    # do all spatial comparisons and store them in a big list'o'lists to be plotted in the next code chunks
    all_LAI_comparisons <- fullSpatialComparison(this_benchmark, all_LAI_Maps, all_LAI_Trends, all_LAI_Seasonal, settings$reference_sim_name)
...
if(do_global_BA){
  ...
  if(length(BA_data_Fields) == 0) {
    ...
    # do all spatial comparisons and store them in a big list'o'lists to be plotted in the next code chunks
    all_BA_comparisons <- fullSpatialComparison(this_benchmark, all_BA_Maps, all_BA_Trends, all_BA_Seasonal, settings$reference_sim_name)
...
if(do_Pan_Biomass){
  ...
  this_benchmark_simulation <- "tellus"
  ...
  for(this_sim_Source in simulation_sources) {
    ...
    # check if file is present (if not don't include this run)
    this_benchmark_run_dir <- file.path(this_sim_Source@dir, this_benchmark_simulation)
...
if(do_Global_Biomass){
  ...
  all_Biomass_comparisons <- fullSpatialComparison(this_benchmark, all_maps = all_Biomass_Maps, reference_simulation =  settings$reference_sim_name)
...
this_soil_N2O_df <- modelSummaryTable(simulations = simulation_sources, 
                                      ...
                                      area_unit = "ha",
                                      unit_multiplier = KG_TO_TG, 
                                      comparison_periods = comparison_period)
