...
//                                 University of Lund                                //
//                                                                                   //
//  Class xtring provides functionality for pointer-free manipulation of character   //
//  strings in C++.                                                                  //
//                                                                                   //
...
//                                                                                   //
//     char copy[100];                                                               //
//     xtring original="text";                                //                        University of Lund, Ekologihuset, Solvegatan 37,           //
//                        22362 Lund, Sweden                                         //
//                        Fax: +46 46 222 4423                                       //
...
//                                                                                   //
//  unsigned long len()                                                              //
//     Returns length of current string in characters (not including trailing null   //
//     character).                                                                   //
//  xtring upper()                                                                   //
//     Returns a new xtring equivalent to the current one, but with lower-case       //
//     alphabetics 'a'-'z' converted to upper case                                   //
//  xtring lower()                                                                   //
//     Returns a new xtring equivalent to the current one, but with upper-case       //
//     alphabetics 'A'-'Z' converted to lower case                                   //
//  xtring printable()                                                               //
//     Returns a new xtring equivalent to the current one, but with non-printable    //
//     characters (ASCII code 0-31) removed                                          //
//  xtring left(unsigned long n)                                                     //
//     Returns a new xtring consisting of the leftmost n characters of the current   //
//     xtring. A null xtring ("") is returned if n<=0; if n>length of the current    //
//     xtring, an identical copy of the current xtring is returned                   //
...
//     copy of the current xtring is returned; if s>=length of the current xtring,   //
//     a null xtring is returned.                                                    //
//  xtring mid(unsigned long s,unsigned long n)                                      //
//     Returns a new xtring consisting of up to n characters, starting at character  //
//     number s of the current xtring. If s<0, the new string starts at character 0  //
//     of the current xtring; if s>=length of the current xtring, a null xtring is   //
//     returned.                                                                     //
//  xtring right(unsigned long n)                                                    //
//     Returns a new xtring consisting of the rightmost n characters of the current  //
//     xtring. A null string is returned if n<=0; if n>length of the current         //
//     xtring, an identical copy is returned.                                        //
//  long find(char* s); long find(char c)                                            //
//     Returns the position of the (first character of the) specified character      //
//     string or character if it occurs as a substring of the current xtring. If     //
//     there are several occurences, the position of the leftmost occurence is       //
//     returned. Returns -1 if string/character not found.                           //
//  long findoneof(char* s)                                                          //
//     Returns the position in this xtring of the first (leftmost) occurrence any    //
//     character forming part of the string pointed to by s. Returns -1 if there     //
//     are no occurrences.                                                           //
//  long findnotoneof(char* s)                                                       //
//     Returns the position in this xtring of the first (leftmost) character NOT     //
//     forming part of the string pointed to by s. Returns -1 if no such character   //
//     is found.                                                                     //
//  double num()                                                                     //
...
//     Expands or contracts the memory allocation to the current xtring object to    //
//     accomodate a string at least n characters in length (not including the        //
//     trailing null byte). The currently stored string may be copied to a new       //
//     location in memory but is not deleted. This function may be useful if you     //
//     intend to write directly to the internal string buffer, whose address is      //
//     returned by casting the xtring object to char*.                               //
//                                                                                   //
//  -------------------------------------------------------------------------------  //
//                                     OPERATORS                                     //
//  -------------------------------------------------------------------------------  //
...
//   { ... };
//
// Either of the above declarations will produce a class including the following
// public functions and member variables:
//
// void initarray(unsigned int nitem)
//   Clears list array (if not empty) and fills it with nitem MyObjectType objects.
//
// MyObjectType& createobj()
//   Creates a new object of type MyObjectType and returns a reference to it.
//
// bool firstobj()
//   Causes the internal object pointer (pthisitem) to point to the first MyObjectType
//   object in the list array. Returns false if the list array is empty.
//
// bool isobj
//   This variable (NB: not a function) is true whenever the internal object pointer
//   points to a MyObjectType object, false otherwise (including when the list array
//   is empty).
//
// bool nextobj()
//   Causes the internal object pointer to point to the next MyObjectType object in the
//   list array. Returns false if the last item has already been reached.
//
// MyObjectType& getobj()
//   Returns a reference to the object currently pointed to by the internal object
//   pointer. Do not call this function unless the pointer is pointing to a valid
//   object (isobj=true).
//
// unsigned int nobj
...
//   Overload of the array brackets operator which returns a reference to the i'th
//   MyObjectType item in the list array. Do not call unless the internal object
//   pointer is pointing to a valid object. Note that this function does NOT affect
//   the value of the internal pointer.
//
...
//
// void initarray(unsigned int nitem)
//   Clears list array (if not empty), resets id counter to 0 and fills list array with
//   nitem MyObjectType objects.
//
// MyObjectType& createobj()
//   Creates a new object of type MyObjectType and returns a reference to it. The id
//   member of the new object is set to the value of a counter which has initial value
//   0 and is incremented (by 1) on each subsequent call to createobj.
//
// bool firstobj()
//   Causes the internal object pointer (pthisitem) to point to the first MyObjectType
//   object in the list array. Returns false if the list array is empty.
...
//
// void killall()
//   Clears the entire list array, releases dynamic memory and resets the id counter
//   to 0.
//
// MyObjectType& operator[](unsigned int i)
//   Overload of the array brackets operator which returns a reference to the i'th
...
// public functions and member variables:
//
// MyObjectType& createobj(MyRefType& ref)
//   Creates a new object of type MyObjectType and returns a reference to it. The
//   constructor function MyObjectType(unsigned int,MyRefType&) is called, with the
//   unsigned int argument set to the value of a counter which has initial value 0 and
//   is incremented (by 1) on each subsequent call to createobj; ref is sent as the
//   MyRefType& argument to the constructor function.
//
// bool firstobj()
//   Causes the internal object pointer (pthisitem) to point to the first MyObjectType
//   object in the list array. Returns false if the list array is empty.
...
// public functions and member variables:
//
// MyObjectType& createobj(MyRefType1& ref1,MyRefType2& ref2)
//   Creates a new object of type MyObjectType and returns a reference to it. The
//   constructor function MyObjectType(unsigned int,MyRefType1&,MyRefType2&) is called,
//   with the unsigned int argument set to the value of a counter which has initial
//   value 0 and is incremented (by 1) on each subsequent call to createobj; ref1 and
//   ref2 are sent as the MyRefType& arguments to the constructor function.
//
// bool firstobj()
//   Causes the internal object pointer (pthisitem) to point to the first MyObjectType
//   object in the list array. Returns false if the list array is empty.
...
// public functions and member variables:
//
// MyObjectType& createobj(MyRefType1& ref1,MyRefType2& ref2,MyRefType3& ref3)
//   Creates a new object of type MyObjectType and returns a reference to it. The
//   constructor function MyObjectType(unsigned int,MyRefType1&,MyRefType2&,
//   MyRefType3&) is called, with the unsigned int argument set to the value of a
//   counter which has initial value 0 and is incremented (by 1) on each subsequent
//   call to createobj; ref1, ref2 and ref3 are sent as the MyRefType& arguments to
...
//
// Member function: void init()
//   Should be called before any other operations on a Timer object
//
// Member function: void settimer([double duration])
//   Commences timing: optional parameter duration gives number of seconds to
//   "finished" status
//
// Member function: void setprogress(double progress)
//   Specifies fractional progress (in range 0-1) towards "finished" status (modifies
//   time remaining to "finished")
//
// Member function: double getprogress()
...
//   char* str                   elapsed time in the form "hh:mm:ss"
...
public:
	long timemilli() {
		...
		long newclock=(double)clock()/(double)CLOCKS_PER_SEC*1000.0;
	...
	void init() {
		m_clock=0;
		lastclock=(double)clock()/(double)CLOCKS_PER_SEC*1000.0;
		timebase=timemilli();
		settimer(3600.0);
	}
	...
	void update() {
		...
		elapsed.seconds=(int)(elapsed.time-elapsed.hours*3600-
			elapsed.minutes*60);
		elapsed.milliseconds=elapsed.time-elapsed.hours*3600-elapsed.minutes*60-
			elapsed.seconds;
		...
		remaining.milliseconds=remaining.time-remaining.hours*3600-
...
// UNIXTIME
//
// Writes date and time in standard Unix format (e.g. "Tue Nov 06 10:17:47 2001")
// to an xtring argument
...
// READFOR
//
// Function synopsis: bool benutil::readfor(FILE* strm, xtring format, ...)
//
...
//      the exponent 0 (i.e., 100 = 1) is assumed.
//
//   Examples:
//
...
// FILEEXISTS
//
// Returns true if the specified filename exists and can be opened for reading,
// otherwise false
