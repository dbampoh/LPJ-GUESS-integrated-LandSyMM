...
typedef enum itemtype {PLIB_STRING,PLIB_INT,PLIB_DOUBLE,PLIB_GROUP,PLIB_SET,
	PLIB_BOOLEAN,PLIB_FLAG};
typedef enum wordtype {WORD_IDENTIFIER,WORD_STRING,WORD_NUMBER,WORD_OPENPARENTHESIS,
	WORD_CLOSEPARENTHESIS,WORD_GROUP,WORD_NOWORD};
typedef enum errortype {PLIB_NOERROR,PLIB_INVALIDNUMBER,PLIB_OUTOFMEMORY,PLIB_ENDOFFILE,
	...
	PLIB_OUTOFRANGE,PLIB_EXPECTOPENPARENTHESIS,PLIB_ABORT};
...
static Pliblist* pthislist,*pglist;
static char currch;
static int lineno;
static Plibgroup* pfirstgroup;
...
void helpout(xtring& identifier,xtring& help) {
	...
	const int TABWIDTH=16;
	const int MAXHELPSTRING=62; // maximum length of 'help' excluding terminating \0
	int i;
	xtring helpcpy;
	xtring tabstring="                ";
		// NB: should be same number of spaces as the value of TABWIDTH
	xtring output;
	int len;

	if (*help)
		helpcpy=help.left(MAXHELPSTRING);
	else helpcpy="(no help available)";

	len=identifier.len();
	if (len<TABWIDTH) output=identifier+tabstring.left(TABWIDTH-len)+helpcpy+"\n";
...
bool declareitem(xtring identifier,xtring* param,int maxlen,int callback) {
	...
	return declareitem(identifier,param,maxlen,callback,"");
}
...
bool declareitem(xtring identifier,int* param,int min,int max,int nparam,int callback) {

	return declareitem(identifier,param,min,max,nparam,callback,"");
}
...
bool declareitem(xtring identifier,bool* param,int nparam,int callback) {
	...
	return declareitem(identifier,param,nparam,callback,"");
}
...
bool declareitem(xtring identifier,bool* param,int callback) {
	...
	return declareitem(identifier,param,callback,"");
}
...
bool declareitem(xtring identifier,int id,int callback) {
	...
	return declareitem(identifier,id,callback,"");
}
...
void readto(xtring& string,char* termlist,FILE*& in) {
...
void getnextword(Plibword& plibword,FILE*& in,errortype& err) {
	...
	do {
		...
		else if (currch>='a' && currch<='z' || currch>='A' && currch<='Z') { // identifier
			char termlist[]=" \t\n-+.\"\'()!";
...
void loadscript(FILE*& in,Plibword*& pfirstword,errortype& err,bool isgroup,
	Plibgroup*& pfirstgroup,bool& firstgroup,Plibgroup*& plastgroup,Plibword& plibword) {
	...
	while (pword) {
		...
		if (pword->type==WORD_GROUP) {
			...
			case PLIB_STRING:
				...
				if (pword->string.len()>pitem->maxlen-1)
