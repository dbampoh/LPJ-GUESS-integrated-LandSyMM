...
void aet_water_stress(Patch& patch, Vegetation& vegetation, const Day& day) {
	...
	for (int p=0; p<npft; p++) {
		...
		if(spft.active)
		{
			...
			ppft.wstress = ppft.wsupply < patch.wdemand && !negligible(ppft.phen) && ((patch.wdemand-ppft.wsupply)>1.0e-10);
...
//
	...
	double pet_patch= pet_patch = patch.wdemand_day * patch.fpc_total + patch.intercep +
