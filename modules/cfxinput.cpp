...
insoltype cf_standard_name_to_insoltype(const std::string& standard_name) {
	if (standard_name == "surface_downwelling_shortwave_flux_in_air" ||
	    ...
	    standard_name == "Downwelling solar flux at surface" ||
		...
		return NOINSOL; // To avoid compiler warning
	}
}
...
void check_temp_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
	if ((cf_var->get_standard_name() != "air_temperature") && (cf_var->get_long_name() != "near-surface temperature") &&
			...
			(cf_var->get_long_name() != "Reference height temperature") &&
...
void check_prec_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
	...
	else if (cf_var->get_long_name() == "Total (convective and large-scale) precipitation rate (liq + ice)") {
		cf_var->ssr_test_precip_units();
	}
	else {
...
void check_insol_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
	if (cf_var->get_standard_name() != "surface_downwelling_shortwave_flux_in_air" &&
		...
		cf_var->get_long_name() != "Downwelling solar flux at surface") {
	...
	if (cf_var->get_standard_name() == "cloud_area_fraction") {
		...
		std::string units = cf_var->get_units();
		if (!((units.compare("W m-2")==0) || (units.compare("W/m^2")==0) || (units.compare("W/m2")==0))) {
			fail("Insolation variable given as radiation but unit doesn't seem to be in W m-2 or W/m^2 or W/m2");
		}
	}
}
...
void check_relhum_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
	...
	if (cf_var->get_standard_name() != standard_name && cf_var->get_long_name() != "Reference height relative humidity") {
		fail("Relative humidity variable should have standard name %s ",standard_name);
	}
	    
    // SSR: GGCMI
//    cf_var->ssr_test_relhum_units();

}

// Verifies that a CF variable with wind data contains what we expect
void check_wind_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
	const char* standard_name = "wind_speed";
	const char* long_name = "10m wind speed";
	if (cf_var->get_standard_name() != standard_name && cf_var->get_long_name() != long_name) {
		fail("Wind variable should have standard name %s and/or long name %s",standard_name,long_name);
	}
	if (cf_var->get_units() != "m s-1" && cf_var->get_units() != "m/s") {
		fail("Wind data must be given in m s-1 or m/s !");
	}
}
	...
	,spinup_climate_nyears(NYEAR_SPINUP_DATA)
{
	...
	declare_parameter("nodetrending", &nodetrending);
	declare_parameter("searchradius", &searchradius, 0, 100, "If specified, cf data will be searched for in a circle");
	declare_parameter("fixed_spinup_climate", &fixed_spinup_climate, 0, 1, "Use spinup climate for the whole run (1), (0) normal run");
	declare_parameter("fixed_ndep", &fixed_ndep, 0, 3, "Use ndep of fixed_ndep_year (def. 1850) for the whole run (1=for the whole run, 2=starting with that year and onward, 3=before that year), (0) time varying ndep");
	declare_parameter("fixed_ndep_year", &fixed_ndep_year, 1850, 2100, "ndep year to use for fixed_ndep>0 (def. 1850) [1850..2100]");
	
	declare_parameter("extend_climate", &extend_climate, 0, 2, "Extend the climate forward using the last 'extend_climate_nyears' years of climate (1=T not detrended,2=T detrended), (0) normal run w/o extending");
	...
	// NOTE: GCP only wants to use 1901..1920 as spinup climate, was left at 30 years in GCP_2016!
...
// SSR: Avoid memory leaks due to multi-part capability
void CFXInput::reset() {
	delete cf_temp;
	delete cf_prec;
	delete cf_insol;
...
bool CFXInput::create_gridlist_from_cflist(xtring& file_gridlist, xtring& file_gridlist_lonlat) {

	std::ifstream ifs(file_gridlist, std::ifstream::in);

//	if (!ifs.good()) fail("CFXInput::init: could not open %s for input",(char*)file_gridlist);

	bool convlonlat=false;
	if (!ifs.good()) {
		dprintf("CF gridlist not good, opening trying to open lon-lat\n");
		ifs.open(file_gridlist_lonlat, std::ifstream::in);
	}
	if (ifs.good()) {
		convlonlat=true;
	} else {
		fail("CFXInput::init: could not open %s for input",(char*)file_gridlist_lonlat);
	}

	std::vector<double> lons; // can hold 0.25x0.25 grid
	std::vector<double> lats; // can hold 0.25x0.25 grid
	if (convlonlat) {
		...
		double lon,lat;
		size_t rlat=0;
		for (size_t rlon=0; rlon< 1440; rlon++) {
			try {
		...
		size_t rlon=0;
		for (size_t rlat=0; rlat< 800; rlat++) {
			try {
				cf_temp->get_coords_for(rlon, rlat, lon, lat);
				lats.push_back(lat);
			} catch(...) {
			...
			if (iss >> rlon >> rlat) {
				...
				if (convlonlat) {
					...
					if (get_index_for_coords(rlon, rlat, ilon, ilat, lons, lats, searchradius)) {
						...
						fail("ERROR: could not find netcdf index for (%g,%g)", rlon, rlat);
					}
				} else {
					c.rlat = (int) rlat;
					c.rlon = (int) rlon;
...
//double CFXInput::parse_spatial_resolution() {
//
//	double dif_lon, dif_lat, lon, lat, lon2, lat2;
//
//	if (cf_temp->is_reduced()) {
//		return 0.5; // FIXME: what to return, lets return our general 0.5deg!
//	}
//
...
//	if(!(fabs(dif_lon - dif_lat) < 0.000001))
...
// SSR: GGCMI
void CFXInput::push_filename(std::queue<xtring>* file_list, xtring varname, int f) {
	
	// Get name of this parameter with integer at end (e.g., file_temp1)
	char this_param [50];
	sprintf(this_param, "file_%s%d", (const char*)varname, f);
	if (param.isparam(this_param)) {
		file_list->push(param[this_param].str);
		
	// If that's not a specified parameter, then there should at least be e.g. file_temp. Add that only as the first value in the queue; for any others, just add a blank.
	} else if (f==1) {
		sprintf(this_param, "file_%s", (const char*)varname);
		file_list->push(param[this_param].str);
	} else {
		file_list->push("");
	}
...
// SSR: GGCMI
void CFXInput::get_files() {
	
	filelist_temp = new std::queue<xtring>;
	filelist_prec = new std::queue<xtring>;
	filelist_insol = new std::queue<xtring>;
	filelist_wetdays = new std::queue<xtring>;
	filelist_pres = new std::queue<xtring>;
	filelist_specifichum = new std::queue<xtring>;
	filelist_wind = new std::queue<xtring>;
	filelist_relhum = new std::queue<xtring>;
	filelist_min_temp = new std::queue<xtring>;
	filelist_max_temp = new std::queue<xtring>;
	
	for (int f=1; f<=max_nfiles; f++) {

		push_filename(filelist_temp, "temp", f);
		push_filename(filelist_prec, "prec", f);
		push_filename(filelist_insol, "insol", f);
		push_filename(filelist_wetdays, "wetdays", f);
		push_filename(filelist_pres, "pres", f);
		push_filename(filelist_specifichum, "specifichum", f);
		push_filename(filelist_wind, "wind", f);
		push_filename(filelist_relhum, "relhum", f);
		push_filename(filelist_min_temp, "min_temp", f);
		push_filename(filelist_max_temp, "max_temp", f);
...
// SSR: GGCMI
xtring CFXInput::get_front_file(std::queue<xtring>* file_list) {
	if (file_list->empty()) {
		fail("File list is empty");
	}
	xtring front_file = file_list->front();
	file_list->pop();
	return front_file;
}

// SSR: GGCMI
void CFXInput::setup_multipart() {
	which_file = 0;
	get_files();
}

// SSR: GGCMI
bool CFXInput::get_next_GridcellOrderedVariable_climate() {
	
	which_file += 1;
	
	if (which_file > max_nfiles) {
		return false;
	}
	
	// Get next filename
	xtring file_temp = get_front_file(filelist_temp);
	xtring file_prec = get_front_file(filelist_prec);
	xtring file_insol = get_front_file(filelist_insol);
	xtring file_wetdays = get_front_file(filelist_wetdays);
	xtring file_pres = get_front_file(filelist_pres);
	xtring file_specifichum = get_front_file(filelist_specifichum);
	xtring file_wind = get_front_file(filelist_wind);
	xtring file_relhum = get_front_file(filelist_relhum);
	xtring file_min_temp = get_front_file(filelist_min_temp);
	xtring file_max_temp = get_front_file(filelist_max_temp);
	...
	try {
		cf_temp = new GridcellOrderedVariable(file_temp, param["variable_temp"].str);
		cf_prec = new GridcellOrderedVariable(file_prec, param["variable_prec"].str);
		cf_insol = new GridcellOrderedVariable(file_insol, param["variable_insol"].str);

		if (file_wetdays != "") {
			cf_wetdays = new GridcellOrderedVariable(file_wetdays, param["variable_wetdays"].str);
		}

		if (file_min_temp != "") {
			cf_min_temp = new GridcellOrderedVariable(file_min_temp, param["variable_min_temp"].str);
		}

		if (file_max_temp != "") {
			cf_max_temp = new GridcellOrderedVariable(file_max_temp, param["variable_max_temp"].str);
		}

		if (file_pres != "") {
			cf_pres = new GridcellOrderedVariable(file_pres, param["variable_pres"].str);
		}

		if (file_specifichum != "") {
			cf_specifichum = new GridcellOrderedVariable(file_specifichum, param["variable_specifichum"].str);
		}

		if (file_relhum != "") {
			cf_relhum = new GridcellOrderedVariable(file_relhum, param["variable_relhum"].str);
		}

		if (file_wind != "") {
			cf_wind = new GridcellOrderedVariable(file_wind, param["variable_wind"].str);
		}
	...
	// all_variables() specifically refers to climate variables
...
void CFXInput::init() {
	...
	// SSR: GGCMI: Get (first) climate variables. Only needed for stuff that happens in this function.
	get_files();
	get_next_GridcellOrderedVariable_climate();
	
	// SSR: Do not allow use of cfxinput if using one of Matt's firemodels
	if (firemodel >= SPITFIRE) {
		fail("firemodel %d not compatible with cfx input", firemodel);
	}
	...
	// SSR: GGCMI: Contents of popdens and Ndep files are compared to climate files inCFXInput::get_next_GridcellOrderedVariable_climate()

	// Read list of localities and store in gridlist member variable

	// Retrieve name of grid list file as read from ins file
	xtring file_gridlist=param["file_gridlist_cf"].str;
	xtring file_gridlist_lonlat=param["file_gridlist"].str;

	if(file_gridlist != "" || file_gridlist_lonlat != "") {
		create_gridlist_from_cflist(file_gridlist, file_gridlist_lonlat);
	}
	...
	if (firsthistyear < 0) {
		...
		dprintf("firsthistyear not specified; using first year in temperature data (%d)\n", firsthistyear);
	}
	if (lasthistyear < 0) {
		lasthistyear = cf_temp->get_date_time(cf_temp->get_timesteps()-1).get_year();
		dprintf("lasthistyear not specified; using last year in temperature data (%d)\n", lasthistyear);
	}
	if (firstoutyear < 0) {
	...
	// SSR: GGCMI: Need to reset file list for first gridcell, because we popped off the first one above.
	get_files();
}
...
bool CFXInput::getgridcell(Gridcell& gridcell) {
	...
	while (current_gridcell != gridlist.end() && !gridfound) {
		...
		if (!gridfound) {
			dprintf("\nWarning: skipping to next gridcell, could not find/read data for (%g,%g)!\n", lon, lat); // SSR: Fails instead of warning - DKB: warns instead of failing - original behaviour
			current_gridcell++;
			continue;
			//return false; // DKB: skip to next gridcell 
		}
	}
	...
	if (restart&&(state_year < climate_first_year))
		sync_first_year = state_year;
	...
	if (extend_climate) {
		...
		if (restart && state_year-1 > extend_climate_startyear) {
			...
			year_offset = state_year-1 - extend_climate_startyear - 1; // -1 since there will be an extclim_data_nextyear later
	...
	if (which_file==1) {
		dprintf("\nCommencing simulation for gridcell at (%g,%g) (index %d,%d) climate (%g,%g)\n", current_gridcell->grid_lon, current_gridcell->grid_lat, current_gridcell->rlon, current_gridcell->rlat, lon, lat);
	...
	// SSR: Make sure all files start on the same day
	DateTime dt_temp = cf_temp->get_date_time(0);
	if ((cf_prec && !GuessNC::datetimes_equiv_ymd(cf_prec->get_date_time(0), dt_temp)) ||
		(cf_insol && !GuessNC::datetimes_equiv_ymd(cf_insol->get_date_time(0), dt_temp)) ||
		(cf_wetdays && !GuessNC::datetimes_equiv_ymd(cf_wetdays->get_date_time(0), dt_temp)) ||
		(cf_min_temp && !GuessNC::datetimes_equiv_ymd(cf_min_temp->get_date_time(0), dt_temp)) ||
		(cf_max_temp && !GuessNC::datetimes_equiv_ymd(cf_max_temp->get_date_time(0), dt_temp)) ||
		(cf_pres && !GuessNC::datetimes_equiv_ymd(cf_pres->get_date_time(0), dt_temp)) ||
		(cf_specifichum && !GuessNC::datetimes_equiv_ymd(cf_specifichum->get_date_time(0), dt_temp)) ||
		(cf_relhum && !GuessNC::datetimes_equiv_ymd(cf_relhum->get_date_time(0), dt_temp)) ||
		(cf_wind && !GuessNC::datetimes_equiv_ymd(cf_wind->get_date_time(0), dt_temp))) {
		fail("Not all climate files start on the same date!");
	}
	
	// SSR: Do not allow firsthistyear < first year in forcing file
	if (!restart && which_file==1 && cf_temp->get_date_time(0).get_year() > firsthistyear) {
		fail("When not restarting, firsthistyear (%d) must be >= first year in temperature file (%d)", firsthistyear, cf_temp->get_date_time(0).get_year()) ;
	}

	// SSR: Deal with firsthistyear > first year in forcing file
	int historic_timestep_init = 0;
	while (cf_temp->get_date_time(historic_timestep_init).get_year() < firsthistyear) {
		++historic_timestep_init;

		if (historic_timestep_init >= cf_temp->get_timesteps()) {
			fail("Failed to get initial historic_timestep_init");
		}
	}
	
	// SSR: These need to start at one below what they should be (e.g., if firsthistyear == first year in file, they should be -1 instead of 0) because before the data are read, the timestep is incremented.
	historic_timestep_temp = historic_timestep_init - 1;
	historic_timestep_prec = historic_timestep_init - 1;
	historic_timestep_insol = historic_timestep_init - 1;
	historic_timestep_wetdays = historic_timestep_init - 1;
	historic_timestep_min_temp = historic_timestep_init - 1;
	historic_timestep_max_temp = historic_timestep_init - 1;
	historic_timestep_pres = historic_timestep_init - 1;
	historic_timestep_specifichum = historic_timestep_init - 1;
	historic_timestep_relhum = historic_timestep_init - 1;
	historic_timestep_wind = historic_timestep_init - 1;
...
bool CFXInput::load_data_from_files(double& lon, double& lat) {
	...
	if(cf_temp->is_reduced() || param["file_gridlist_cf"].str != "") {
	...
	if (cf_temp->is_reduced()) {
		if (!cf_temp->load_data_for(landid) ||
		    ...
		    (cf_mNHxwetdep && true) ||
		    (cf_mNOywetdep && true)) {
			dprintf("Failed to load climate data for (%d) from NetCDF files, skipping.\n", landid); // SSR: Fails instead of warning - DKB - warn instad of failing - orginal behaviour
			return false; // DKB - skip to next gridcell 
		}
	}
	else {
		if (!cf_temp->load_data_for(rlon, rlat) ||
			...
			dprintf("Failed to load climate data for (%.2f,%.2f) [%d, %d] from NetCDF files, skipping.\n", current_gridcell->grid_lon, current_gridcell->grid_lat, rlon, rlat); // SSR: Fails instead of warning - DKB: warn instead of failing - original behaviour
			return false; // DKB - skip to next gridcell
		}
...
void CFXInput::get_yearly_data(std::vector<double>& data,
	...
	if (is_daily(cf_historic)) {
		...
		while (current_day.year == date.year) {
			...
			// In the spinup?
// SSR			if (earlier_day(current_day, calendar_year, cf_historic->get_date_time(0)) ||
			if (calendar_year < firsthistyear || // SSR
...
// SSR			else if (calendar_year <= last_date.get_year()) {
			else if (calendar_year <= lasthistyear) { // SSR
				// Historical period
				...
				fail("extend climate not implemented for daily data");
			}
...
void CFXInput::populate_daily_arrays(Gridcell& gridcell) {
	...
	if ( !is_daily(cf_temp) && weathergenerator == GWGEN ) {
		...
		if (cf_wind) {
			populate_daily_array(dwind, spinup_wind, cf_wind, extclim_pres, historic_timestep_wind, 0);
		}
	...
	if (extend_climate && date.get_calendar_year() > extend_climate_startyear) {
		...
		if (date.get_calendar_year() > (extend_climate_startyear+1)) { /// only need to advance after the 1st year
			extclim_data_nextyear();
		}
		dprintf("%d idx %d/%d cycle year %d\n",date.get_calendar_year(), extclim_temp.thisyear, extclim_temp.nbr_years(), extclim_temp.years[extclim_temp.thisyear]);
	}
...
bool CFXInput::getclimate(Gridcell& gridcell) {
	...
	if (which_file==0) {
		if (!get_next_GridcellOrderedVariable_climate()) {
			reset();
			return false;
		}
		else if (!getgridcell(gridcell)) {
			reset();
			return false;
		}
	}
	...
	// If you're at the end of this climate period, check to see if there's another period. If so, read the files for that period and update last_date.
	if (later_day(date, last_date) && date.get_calendar_year() <= lasthistyear) {
		if (get_next_GridcellOrderedVariable_climate()) {
			last_date = last_day_to_simulate(cf_temp);
	...
	if (date.day == 0) {
		...
		if (tmute.getprogress()>=1.0) {
			...
			int yearsdone_thiscell;
			if (restart) {
				years_to_simulate = lasthistyear - state_year + 1;
				yearsdone_thiscell = date.get_calendar_year() - state_year;
			} else {
				int historic_years = lasthistyear - firsthistyear + 1;
				years_to_simulate = nyear_spinup + historic_years;
				yearsdone_thiscell = date.year;
			}
			
			int cells_done = distance(gridlist.begin(), current_gridcell);

			double progress=(double)(cells_done*years_to_simulate+yearsdone_thiscell)/
...
void CFXInput::load_spinup_data(const GuessNC::CF::GridcellOrderedVariable* cf_var,
	...
	// SSR: First spinup year should be firsthistyear, to match previous behavior
	while (cf_var->get_date_time(timestep).get_year() < firsthistyear || // SSR
		   ((daily && !first_day_of_year(cf_var->get_date_time(timestep))) ||
