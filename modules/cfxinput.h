...
public:
	...
	// SSR: GGCMI
	void push_filename(std::queue<xtring>* file_list, xtring varname, int f);
	void get_files();
	xtring get_front_file(std::queue<xtring>* file_list);
	bool get_next_GridcellOrderedVariable_climate();
...
//	double parse_spatial_resolution();
	...
	int max_nfiles = 4;
	int which_file = 0;
	std::queue<xtring>* filelist_temp;
	std::queue<xtring>* filelist_prec;
	std::queue<xtring>* filelist_insol;
	std::queue<xtring>* filelist_wetdays;
	std::queue<xtring>* filelist_pres;
	std::queue<xtring>* filelist_specifichum;
	std::queue<xtring>* filelist_wind;
	std::queue<xtring>* filelist_relhum;
	std::queue<xtring>* filelist_min_temp;
	std::queue<xtring>* filelist_max_temp;
