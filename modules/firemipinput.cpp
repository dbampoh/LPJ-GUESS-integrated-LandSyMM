...
REGISTER_INPUT_MODULE("firemip", FireMIPInput)
...
// MF: SPITFIRE addition for reading a(Nd) from a text file
void read_hum_ign_from_file(double lon, double lat, xtring fname, const char* format, double &aNd) {


	double dlon, dlat;
	FILE* in = fopen(fname, "r");
	if (!in) {
		fail("read_hum_ign_from_file: could not open %s for input", (char*) fname);
	}

	bool foundgrid = false;
	while (!feof(in) && !foundgrid) {
		readfor(in, format, &dlon, &dlat, &aNd);
		// below condition seems to fail, perhaps due to precision issues
		//foundgrid = equal(lon, dlon) && equal(lat, dlat);
		foundgrid = fabs(lon - dlon) < 0.000001 && fabs(lat - dlat) < 0.000001;
	}

	fclose(in);
	if (!foundgrid) {
		fail("read_hum_ign_from_file: could not find record for (%g,%g) in %s", lon, lat, (char*) fname);
	}
}
...
bool cf_read_lightning(const GuessNC::CF::GridcellOrderedVariable* cf_lightning, std::vector<LightningStrokes> &lightning_strokes) {
	int ntimes = cf_lightning->get_timesteps();
	//dprintf("ntimes %i\n", ntimes);
	if (ntimes == 0)
		return false;

	lightning_strokes.clear();
	// lets assume we have yearly data
	LightningStrokes tmp_lightning;
	for (int idx=0; idx < ntimes; idx++) {
		DateTime cf_date;
		cf_date = cf_lightning->get_date_time(idx);
		tmp_lightning.year = cf_date.get_year();
		double strokes = cf_lightning->get_value(idx);
		if (std::isnan(strokes)||std::isinf(strokes)||negligible(strokes - (-99999.0))) // LIS/ODT lightning has -99999 as missing value
			strokes = 0.0;
		tmp_lightning.strokes[cf_date.get_month()-1] = strokes;
		//dprintf("tmp_lightning.year = %i, strokes = %f, month = %i\n", tmp_lightning.year, strokes, cf_date.get_month());
		if(cf_date.get_month() == 12) lightning_strokes.push_back(tmp_lightning);
	}
	...
	,lightningcycleoffset(0)
{
	...
	// Lighting options for before the lightning time series starts
	declare_parameter("ifcyclelightning", &ifcyclelightning, "Whether to cycle the lightning time series data before the time series starts (if false just use the first year)");
	declare_parameter("lightningcycleoffset", &lightningcycleoffset, 1, 10000, "Whether to cycle the lightning time series data before the time series starts (if false just use the first year)");
...
bool FireMIPInput::create_gridlist_from_cflist(xtring& file_gridlist, xtring& file_gridlist_lonlat) {

	std::ifstream ifs(file_gridlist, std::ifstream::in);

//	if (!ifs.good()) fail("FireMIPInput::init: could not open %s for input",(char*)file_gridlist);
	...
	if (ifs.good()) {
		...
		fail("FireMIPInput::init: could not open %s for input",(char*)file_gridlist_lonlat);
	}
	...
	if (convlonlat) {
		...
		size_t rlon=0;
		for (size_t rlat=0; rlat< 720;rlat++) {
			try {
				cf_temp->get_coords_for(rlon, rlat, lon, lat);
				lats.push_back(lat);
			} catch(...) {
...
//double FireMIPInput::parse_spatial_resolution() {
//
...
void FireMIPInput::init() {

	// Read CO2 data from file
	co2.load_file(param["file_co2"].str);

	// MF also read A(Nd) data
	file_aNd = param["file_aNd"].str;
	...
	try {
		...
		if (param.isparam("file_light")&&param["file_light"].str != "") {
			cf_light = new GridcellOrderedVariable(param["file_light"].str, param["variable_light"].str);
		}
	...
	if (cf_light) {
			if (!cf_light->same_spatial_domain(*cf_temp)) {
				fail("lightning %s has not the same spatial domain as climate data",(char*) param["file_light"].str);
			};
		}
	...
	// MF: check available variables for SPITFIRE Nesterov calculations
	if(firemodel == SPITFIRE) {

		// In every case we need maximum temperature
		if(!cf_max_temp) {
			fail("maximum temperature required for SPITFIRE, but this has not been specified.\n");
		}
		else {
			dprintf("Got maximum temperature for SPITFIRE Nesterov calculation.\n");
		}

		// Now check for some measure of humidity for calculating dewpoint temperature, or fall back to tmin-4.
		if(cf_relhum) {
			dprintf("Calulating dewpoint temperature for Nesterov index using relative humidity.\n");
		}
		else if(cf_specifichum && cf_pres) {
			dprintf("Calulating dewpoint temperature for Nesterov index using relative humidity via specific humidity and pressure.\n");
		}
		else if(cf_min_temp) {
			dprintf("Approximating dewpoint temperature for Nesterov index with tmin-4.\n");
		}
		else {
			fail("Insufficient climate fields to calculate Nesterov index.\n");
		}
...
void FireMIPInput::spinup_data_nextyear() {
...
void FireMIPInput::extclim_data_nextyear() {
...
bool FireMIPInput::getgridcell(Gridcell& gridcell) {
	...
	while (current_gridcell != gridlist.end() && !gridfound) {
		...
		if (!gridfound) {
			//fail("\nError: could not find/read data for (%g,%g)!\n", lon, lat); // SSR: Fails instead of warning
			dprintf("\nError: could not find/read data for (%g,%g)!\n", lon, lat); // DKB
		}
	}
	...
	// MF: For SPITFIRE get a(Nd)
	if (param["file_aNd"].str != "") {
		read_hum_ign_from_file(lon, lat, file_aNd, "f,f,f", gridcell.aNd);
		// dprintf("gridcell.aNd = %f\n", gridcell.aNd);
	}
	else if(ignitionmode == BOTH || ignitionmode == HUMAN) {
		dprintf("Fail:: No a(Nd) file found even though human ignitions are specified. Aborting.\n");
		abort();
	}
	else {
		gridcell.aNd = 0.0;
	...
	// MF: read lightning
	if (cf_light) {
		cf_read_lightning(cf_light, lightning_strokes);
	}
...
bool FireMIPInput::load_data_from_files(double& lon, double& lat) {
	...
	if (cf_temp->is_reduced()) {
		if (!cf_temp->load_data_for(landid) ||
			...
			(cf_light && true)    || // landid not implemented for the ff. nc's, hence fail
		    (cf_mNHxdrydep && true) ||
		    (cf_mNOydrydep && true) ||
		    (cf_mNHxwetdep && true) ||
		    (cf_mNOywetdep && true)) {
			fail("Failed to load climate data for (%d) from NetCDF files, failing.\n", landid); // SSR: Fails instead of warning
		}
	}
	else {
		if (!cf_temp->load_data_for(rlon, rlat) ||
			...
			fail("Failed to load climate data for (%.2f,%.2f) [%d, %d] from NetCDF files, failing.\n", current_gridcell->grid_lon, current_gridcell->grid_lat, rlon, rlat); // SSR: Fails instead of warning
		}
		...
		// need to handle pop dens differently, seems GCP pop dens has some grid cells missing e.g. -105.25 73.75 pop dens is missing (-999)
		if (cf_light && !cf_light->load_data_for(rlon, rlat)) {
			// will set it to zero later
			dprintf("Failed to load lightning for (%.2f,%.2f) [%d, %d] from NetCDF files, will use 0.0 as lightning.\n", current_gridcell->grid_lon, current_gridcell->grid_lat, rlon, rlat);
		}
...
void FireMIPInput::get_yearly_data(std::vector<double>& data,
	...
	if (is_daily(cf_historic)) {
		...
		while (current_day.year == date.year) {
			...
			else if (calendar_year <= last_date.get_year()) {
				// Historical period
...
void FireMIPInput::populate_daily_array(double* daily,
                                   const XGenericSpinupData& spinup,
                                   GridcellOrderedVariable* cf_historic,
                                   const XGenericSpinupData& extclim,
                                   int& historic_timestep,
...
void FireMIPInput::populate_daily_prec_array(long& seed) {
...
void FireMIPInput::populate_daily_arrays(Gridcell& gridcell) {
	...
	if ( !is_daily(cf_temp) && weathergenerator == GWGEN ) {
		...
		for (int i = 0; i < date.year_length(); ++i) {
			...
			// MF: calculate extra things for SPITFIRE
			if ( firemodel == SPITFIRE ) {
						...
						//dprintf("CLN  dspecifichum[%i] %f \n", i, dspecifichum[i] );
						//dprintf("CLN  dpres       [%i] %f \n", i, dpres[i] );
						//dprintf("CLN  dtmax       [%i] %f \n", i, dmax_temp[i] );
						//dprintf("CLN  dtmin       [%i] %f \n", i, dmin_temp[i] );
						//dprintf("CLN  drelhum     [%i] %f \n", i, drelhum[i] );
						//dprintf("CLN  es1         [%i] %f \n", i, es );
						...
						// MF: Calculate Nesterov index
						double ddew_temp = 0.0;

						if((cf_pres && cf_specifichum) || cf_relhum ) {	// Need to calculate dew point temperature
							// Note relative humidity is already as fraction
							double a = 17.27; // (deg C) constant from wikipedia(!)
							double b = 237.7; // (deg C) constant from wikipedia(!)
							double gamma;
							// if statement to avoid error for zero humidity
							if(drelhum[i] > 0) gamma = (a * dtemp[i]) / (b + dtemp[i]) + log(drelhum[i]);
							else gamma = (a * dtemp[i]) / (b + dtemp[i]) + log(0.1);
							ddew_temp = (b * gamma) / (a - gamma);
						}
						else if(cf_pres && cf_specifichum){
							fail("Calculating dewpoint temperature from pressure and specific humidity not yet implemented.\n ");
						}
						else if(cf_min_temp){
							ddew_temp = (dmin_temp[i] - 4);
						}
						else {
							fail("Cannot calculate dewpoint temperature for Nesterov index.  Please supply either relative humidity; specific humidity and atmospheric pressure; or minimum temperature.\n");
						}

						// calculate NFDI increment provided precipitation < 3mm day^-1 and temp > 0 deg C
						if (dprec[i] < 3.0 && dtemp[i] > 0.0) {
							//dprintf("dmax_temp[i] =%f, dew_point_temp = %f, tmin-4 = %f\n", dmax_temp[i], dew_point_temp, dmin_temp[i]-4);
							double incr = (dmax_temp[i] - ddew_temp) * dmax_temp[i];
							if (incr >= 10000) {
								dprintf("NEST_ERR: Extremely large Nesterov increment = (%f)", incr);
								dprintf("Nesterov_index: Tmax = %f deg C, Rel Humidity = %f %% and Tdewpoint = %f deg C \n", dmax_temp[i], drelhum[i], ddew_temp);
							} else if (incr >= 0.0) {
								dnest[i] = incr + prev_nest;
							} else {
								dprintf("NEST_ERR: Negative increment in Nesterov FDI (%f), possibly inconsistent input data? \n", incr);
								//dprintf("MFDEBUG in calculate_Nesterov_index: Tmax = %f deg C, Rel Humidity = %f %% and Tdewpoint = %f deg C \n", dmax_temp[i], drelhum[i], ddew_temp);
								dprintf("NEST_ERR: Tmax = %f deg C, Tmean = %f deg C and Tdewpoint = %f deg C \n", dmax_temp[i], dtemp[i], ddew_temp);

							}
							//dprintf("Daily Nesterov = %f, increment = %f, previous Nesterov = %f\n", dnest[i], incr, prev_nest);

						} else if (dprec[i] >= 3.0) {
							dnest[i] = 0;
							//dprintf("Precip = %f, Nesterov reset! \n", dprec[i] );
						}
						//dprintf("CLN  dew T       [%i] %f \n", i, dew_point_temp );
						//dprintf("CLN  dprec       [%i] %f \n", i, dprec[i] );
						//dprintf("CLN  dnest       [%i] %f \n", i, dnest[i] );
						prev_nest = dnest[i];


						// MF: Calculate vapour pressure deficit

						// SVP (saturation vapour pressure) - gives hecto pascals
						double SVP_hPa = 6.1121 * exp(dtemp[i]*(18.678-(dtemp[i]/234.5))/(257.14+dtemp[i]));
						double SVP_kPa = SVP_hPa/10;
						//dprintf("CLN  es2         [%i] %f \n", i, SVP_kPa );
						...
						//     = SVP - (RH*SVP)/100
						//     = SVP * (1 - (RH/100))
						double VPD = SVP_kPa * (1 - (drelhum[i]/100));

						//dprintf("temp = %f, rhum = %f, SVP_kPa = %f,VPD = %f\n", Tmean, rhum, SVP_kPa, VPD);

						dvpd[i] = max(0.0,VPD);
						//dprintf("CLN  dvpd        [%i] %f \n", i, dvpd[i] );
...
void FireMIPInput::getlandcover(Gridcell& gridcell) {

	landcover_input.getlandcover(gridcell);
	landcover_input.get_land_transitions(gridcell);
}

bool FireMIPInput::getclimate(Gridcell& gridcell) {
	...
	// MF update lightning on the first day of the month (only if SPITFIRE enabled )
	if(date.dayofmonth == 0 && firemodel == SPITFIRE) {

		double lightning = 0;
		int cyear = date.get_calendar_year();

		// check
		if (lightning_strokes.size()==0)
			fail("no lightning data available, size() == 0");

		// if before lightning time series starts use the first year or cycle through the years
		if (cyear < lightning_strokes[0].year) {
			if(ifcyclelightning) {
				int idx = (cyear + lightningcycleoffset) % lightning_strokes.size() ;
				//if(date.month ==0) dprintf("calendar year = %i, idx = %i, lightning year = %i, simulation year = %i,\n", cyear, idx, lightning_strokes[idx].year, date.year);
				lightning = lightning_strokes[idx].strokes[date.month];
			}
			else{
				lightning = lightning_strokes[0].strokes[date.month];
			}
		}
		// else (note that if the climate time series is longer than the lightning time series
		// then the last lightning year is repeated to the end of the climate time series)
		else {
			int idx = 0 ;
			while (lightning_strokes[idx].year < cyear && idx < (lightning_strokes.size()-1)) {
				idx++;
			}
			//if(date.month ==0) dprintf("calendar year = %i, idx = %i, lightning year = %i,\n", cyear, idx, lightning_strokes[idx].year);
			lightning = lightning_strokes[idx].strokes[date.month];
		}

		gridcell.lightning = lightning;

	}

	// dprintf("c.year = %i, year = %i, month = %i, day = %i, lightning = %f\n", date.get_calendar_year(), date.year, date.month, date.day, gridcell.lightning);
	...
	//MF
	climate.vpd   = dvpd[date.day];
	climate.nest   = dnest[date.day];
	climate.lightning   = dlightning[date.day];

	// MF - for consistency with BLAZE use the variable "gridcell.nesterov_cur", maybe one day use consistent Nesterov
	if(firemodel == SPITFIRE) gridcell.nesterov_cur = climate.nest;
	// dprintf("Daily Nesterov = %f\n", gridcell.nesterov_cur);
...
void FireMIPInput::load_spinup_data(const GuessNC::CF::GridcellOrderedVariable* cf_var,
...
void FireMIPInput::load_extclim_data(const GuessNC::CF::GridcellOrderedVariable* cf_var,
...
std::vector<GuessNC::CF::GridcellOrderedVariable*> FireMIPInput::all_variables() const {
